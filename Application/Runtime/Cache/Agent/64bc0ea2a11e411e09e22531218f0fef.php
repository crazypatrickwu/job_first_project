            ...
            <div class="row-fluid">
                <div class="search-well">
                  <form class="form-inline" action="" metod="get">
                      <input class="input-xlarge" id="appendedInputButton" type="text" name="userid" value="<?php echo remove_xss(I('get.userid'));?>" placeholder="请输入玩家ID进行检索" style="width:75%;margin-left:2px;">
                      <button class="btn" type="button" onclick="location.href='<?php echo U();?>'+'?userid='+$(this).parent().find('input').val();"><i class="icon-search"></i>搜索</button>
                  </form>
                </div>
                <div class="well">
                    <table class="table">
                      ...
                      <tbody>
                        <?php if(empty($game_user_list)): ?><tr>
                               <td colspan="4" align="center" style="text-align: center !important;">没有玩家列表~！</td>
                           </tr>
                        <?php else: ?>
                        <?php if(is_array($game_user_list)): $i = 0; $__LIST__ = $game_user_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$game_user): $mod = ($i % 2 );++$i;?><tr>
                                <td class="center" style="width: 20%;"><?php echo ($game_user['userid']); ?></td>
                                <td class="center" style="width: 30%;"><?php echo ($game_user['nickname']); ?></td>
                                <td class="center" style="width: 30%;">
                                    旗力币：<?php echo ($game_user['THTreasureDB']['score']); ?>(个)
                                    <br>
                                    房卡：<?php echo ($game_user['THTreasureDB']['insurescore']); ?>(个)
