...
<div class="btn-toolbar">
  <button class="btn btn-primary" onClick="location.href='<?php echo U('Agent/addAgent');?>'" style="margin-left:2px;"><i class="icon-plus"></i> 添加代理</button>
  <div class="btn-group">
  </div>
</div>
    ...
    <table class="table">
      ...
      <tbody>
      	<?php if(empty($agentList)): ?><tr>
               <td colspan="4" align="center" style="text-align: center !important;">没有代理数据~！</td>
           </tr>
        <?php else: ?>
        <?php if(is_array($agentList)): $i = 0; $__LIST__ = $agentList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$agent): $mod = ($i % 2 );++$i;?><tr>
                <td class="center" style="width: 10%;"><?php echo ($agent['id']); ?></td>
                <td class="center" style="width: 35%;"><?php echo ($agent['nickname']); ?></td>
                <td class="center" style="width: 35%;"><?php echo ($agent['phone']); ?></td>
