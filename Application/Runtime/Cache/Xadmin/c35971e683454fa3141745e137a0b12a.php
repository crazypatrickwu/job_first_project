...
<div class="pageheader">
    <h1 class="pagetitle">编辑管理员</h1>
    <span class="pagedesc"></span>
</div>

<div id="contentwrapper" class="contentwrapper">
	<form action="<?php echo U('Admin/editAdmin');?>" method="post" class="stdform">
		<input type="hidden" name="id" id="id" value="<?php echo ($adminInfo['id']); ?>" />
		<p>
			<label>账户</label>
			<span class="field">
				<input type="text" name="account" id="account" class="smallinput" value="<?php echo ($adminInfo['admin_account']); ?>" readOnly="true" />
			</span>
		</p>
		<p>
			<label>密码</label>
			<span class="field">
				<input type="password" name="password" id="password" class="smallinput" value="nochange" />
			</span>
		</p>
		<p>
			<label>所属组</label>
			<span class="field">
				<select name="group" id="group">
					<?php if(is_array($group)): $i = 0; $__LIST__ = $group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option <?php if($adminInfo['group_id'] == $g['id']): ?>selected<?php endif; ?> value="<?php echo ($g['id']); ?>"><?php echo ($g['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</span>
		</p>
		<p>
			<label>是否锁定</label>
			<span class="field">
				<input type="radio" name="is_lock" value="0" <?php if($adminInfo['is_lock'] == '0'): ?>checked="checked"<?php endif; ?> /> 否
				<input type="radio" name="is_lock" value="1" <?php if($adminInfo['is_lock'] == '1'): ?>checked="checked"<?php endif; ?>/> 是
			</span>
		</p>
		<p>
			<label></label>
