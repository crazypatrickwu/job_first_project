...
<body class="withvernav">
	...
	<form action="<?php echo U('Admin/addAdmin');?>" method="post" class="stdform">
		<p>
			<label>账户</label>
			<span class="field">
				<input type="text" name="account" id="account" class="smallinput" />
			</span>
		</p>
		<p>
			<label>密码</label>
			<span class="field">
				<input type="password" name="password" id="password" class="smallinput" />
			</span>
		</p>
		<p>
			<label>所属角色</label>
			<span class="field">
				<select name="group" id="group">
					...
					<?php if(is_array($group)): $i = 0; $__LIST__ = $group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g['id']); ?>"><?php echo ($g['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</span>
		</p>
		<p>
