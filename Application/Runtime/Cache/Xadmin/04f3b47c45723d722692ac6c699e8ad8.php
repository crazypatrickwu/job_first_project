...
<body class="withvernav">
	...
	<div class="pageheader">
		<h1 class="pagetitle">角色名更改</h1>
		<span class="pagedesc"></span>
	</div>

	<div id="contentwrapper" class="contentwrapper">
		<form action="<?php echo U('Admin/roleRename');?>" method="post" class="stdform">
			<input type="hidden" name="id" id="id" value="<?php echo ($authInfo['id']); ?>" />
			<p>
				<label>角色名</label>
				<span class="field">
					<input type="text" name="title" id="title" value="<?php echo ($authInfo['title']); ?>" class="smallinput" />
				</span>
			</p>
			<p>
				<label>状态</label>
				<span class="field">
					<input type="radio" name="status" value="1"<?php if($authInfo['status'] == 1): echo chr(32);?>checked="checked"<?php endif; ?>/>可用
					<input type="radio" name="status" value="0"<?php if($authInfo['status'] == 0): echo chr(32);?>checked="checked"<?php endif; ?>/>禁用
				</span>
			</p>
			<p>
				<label></label>
