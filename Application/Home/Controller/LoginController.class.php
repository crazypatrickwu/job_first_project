...
class LoginController extends Controller {
    ...
    public function __construct() {
            // echo md5(sha1('1234') . 'xinmiao2016!QAZ@WSX');exit;
        parent::__construct();
    ...
    public function index() {
        ...
        if (IS_POST) {
            ...
            $agentInfo = M('Agent')->where(array('phone'=>$account,'is_delete'=>0))->field(true)->order('id DESC')->find();
           // dump($agentInfo);die;
            if(empty($agentInfo)){
                die(json_encode(array('code'=>0,'msg'=>'账号不存在')));
            }
            if(think_ucenter_md5($password) != $agentInfo['agent_password']){
                die(json_encode(array('code'=>0,'msg'=>'账号或密码错误')));
            }
            // if ($agentInfo['is_lock'] == '1') {
            //     die(json_encode(array('code'=>0,'msg'=>'账户未被激活')));
            // }
    ...
    public function logout() {
        ...
        redirect(U('Index/index'),3,'<h1>正在退出...</h1>');
